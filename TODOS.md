...
## P2: `dfiles import --from chezmoi` (v1 — plain files)

**What:** Migration tool that reads a chezmoi source directory and converts plain (non-template, non-symlink) files into dfiles format, with module-aware grouping.

**Why:** Most potential dfiles users currently use chezmoi. Without a migration path, switching costs are high enough to block adoption entirely.

**Pros:** Becomes the adoption flywheel. Any chezmoi user can migrate in one command. This is how dfiles grows from personal tool to open source project.

**Cons:** v1 skips templates, private files, executable scripts, and symlinks — users with heavy template usage get a partial migration. The P1 follow-ons below complete the picture.

**Context:** Design doc approved 2026-03-20 at `~/.gstack/projects/dfiles/jstegeman-master-design-20260320-135915.md`. Scope: decode `dot_` prefixes + bare files, infer modules (shell/git/editor/misc), copy to `source/`, generate TOML. New files: `src/chezmoi.rs`, `src/commands/import.rs`. Idempotent re-import; `--dry-run` supported; `--source` override flag.

**Effort:** M (human) → S (CC+gstack)
**Priority:** P2
**Depends on:** dfiles v1 core (modules + template system) being stable

---

## P1 (chezmoi importer): Template conversion

**What:** Convert `.tmpl` files from Go template syntax to Tera during import.

**Why:** Go templates (`{{ .chezmoi.homeDir }}`) cause Tera rendering to fail on `dfiles apply`. Without this, any user with templated dotfiles gets an incomplete migration.

**Pros:** Handles the most common chezmoi templates. Users with shell configs like `.zshrc.tmpl` get a working migration instead of a skip.

**Cons:** Unknown Go template constructs must be left as comments with a migration note. Edge cases in complex templates will require manual cleanup.

**Context:** Known mappings: `{{ .chezmoi.homeDir }}` → `{{ home_dir }}`, `{{ .chezmoi.os }}` → `{{ os }}`, `{{ .chezmoi.hostname }}` → `{{ hostname }}`. Unknown constructs preserved as `{{# TODO: convert: ... #}}`. The `.tmpl` suffix is stripped from the source filename.
...
## P1 (chezmoi importer): `symlink_` target import

**What:** Resolve symlink targets and emit a warning that dfiles copies files, not symlinks.

**Why:** Users with symlinked dotfiles get a silent skip today. Resolving the target gives them a working (if non-symlink) migration.

**Pros:** No broken state — users get the file content even if the symlink semantics are lost.

**Cons:** Symlink semantics are lost. Users who relied on the symlink pointing at a shared file need manual follow-up.

**Context:** Read the symlink target from the chezmoi source file content (chezmoi stores target as file content for `symlink_` entries). Copy the target file if it exists; otherwise skip with a warning.
